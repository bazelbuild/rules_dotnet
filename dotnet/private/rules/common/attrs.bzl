...
load("//dotnet/private:providers.bzl", "DotnetAssemblyCompileInfo", "DotnetAssemblyRuntimeInfo", "DotnetResxInfo")
...
COMMON_ATTRS = {
    ...
    "resx": attr.label_list(
        doc = "A list of `resx_resource` targets whose compiled `.resx` resources are " +
              "embedded in the assembly. Neutral resources are embedded directly and " +
              "per-culture resources are compiled into satellite assemblies " +
              "(`<assembly>.resources.dll`) for localization.",
        providers = [DotnetResxInfo],
        default = [],
        # Compiled `.resx` resources are target-framework agnostic, so - like `resources` -
        # we transition to the default configuration to keep the TFM graphs disconnected.
        cfg = default_transition,
    ),
    "out": attr.string(
