...
let parseGroups (groups: (string * string seq * (string * string) seq) seq) =
    let cache = ConcurrentDictionary<string, Package>()

    groups
    |> Seq.map (fun (group, sources, packages) ->

        let packagesInGroup =
            packages
            |> Array.ofSeq
            |> Array.Parallel.map (fun (name, version) ->
                let found, value = cache.TryGetValue(sprintf "%s-%s-%s" group name version)
                ...
                | false ->
                    ...
                    let package =
                        { name = name
                          ...
                          dependencies =
                            getDependenciesPerTFM tfms (packages |> Seq.map (fun (name, _) -> name)) packageReader
                          overrides = getOverrides name version packageReader
                          frameworkList = getFrameworkList name version packageReader }

                    cache.TryAdd((sprintf "%s-%s-%s" group name version), package) |> ignore
